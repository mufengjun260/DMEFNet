...
from Dataset_within import Dataset as EMGDataset
...
def train_model(dataset_loader, test_dataset_loader, EEGModel, optimizer, criterion, epoch, model_path, logger, tag):
    global total_count
    best_result = 0.
    for epoch in range(epoch):
        ...
        if acc > best_result:
            best_result = acc
            torch.save(EEGModel.state_dict(), model_path + "model.pth")
    logger.info(
...
def main():
    ...
    for i in range(0, 10):
        dataset = EMGDataset(opt.clip_length, opt.step, [i], with_remix=False, get_first_part=True,
                             with_extra_small=False, first_is_train=True, first_part_rate=0.7)
