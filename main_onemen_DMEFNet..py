...
from Dataset_within import Dataset as sEMG_EEG_Dataset
from EM_DCANet_oneman import EMDCANet
...
def main():
    ...
    for i in range(0, 10):
        dataset = sEMG_EEG_Dataset(opt.clip_length, opt.step, [i], with_remix=True, get_first_part=True,
                                   with_extra_small=False, first_is_train=True, first_part_rate=opt.train_rate)
        test_dataset = sEMG_EEG_Dataset(opt.clip_length, opt.step, [i], with_remix=False, get_first_part=False,
                                        with_extra_small=True, first_is_train=True, first_part_rate=opt.train_rate)
        ...
        sEMG_EEG_Model = sEMG_EEG_Model  # device_ids will include all GPU devices by default

        optimizer = torch.optim.Adam(sEMG_EEG_Model.parameters(), lr=0.0001,weight_decay=1e-1)
        ...
        result, count = train_model(dataset_loader, test_dataset_loader, sEMG_EEG_Model, optimizer, criterion, 1500,
                                    model_path, logger,
                                    i)
